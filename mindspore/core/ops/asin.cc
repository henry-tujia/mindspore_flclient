...
abstract::ShapePtr AsinInferShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  (void)CheckAndConvertUtils::CheckInteger("Asin input number", SizeToLong(input_args.size()), kEqual, input_num,
                                           prim_name);
...
AbstractBasePtr AsinInfer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
  ...
  auto type = AsinInferType(primitive, input_args);
  auto shape = AsinInferShape(primitive, input_args);
